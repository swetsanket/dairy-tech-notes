...
const KIND_META = {
  notes: { title: "Notes", blurb: "Semester-wise chapter notes.", icon: "N", cls: "icon-notes" },
  textbook: { title: "Textbook", blurb: "Reference books.", icon: "T", cls: "icon-pill" },
  ppt: { title: "PPT", blurb: "Lecture slides.", icon: "P", cls: "icon-ppt" }
};
...
function loadDb() {
  try {
    ...
    if (raw) {
      ...
      if (parsed && parsed.subjects) return parsed;
    }
  } catch (e) {}
...
function filesView(kind, year, semester) {
  const meta = KIND_META[kind];
  const subs = catalog.subjects.filter((s) => s.year === year && s.semester === semester);
  const blocks = subs.map((subject) => {
    const chapters = subject.chapters.map((ch) => {
      const files = (ch.files || []).filter((f) => f.kind === kind);
      const fileBits = files.length === 0 ? `<span class="muted">Link pending</span>` : files.map((f) => `<button class="ghost" data-open-file="${encodeURIComponent(JSON.stringify(f))}">Open</button>`).join(" ");
      return `<div class="chapter-row"><div><strong>${escapeHtml(ch.name)}</strong></div><div class="inline-actions">${fileBits}</div></div>`;
    }).join("");
    return `<article class="panel subject-block"><div class="subject-head"><h2>${escapeHtml(subject.name)}</h2></div>${chapters}</article>`;
  }).join("");

  return `
    ${crumb([{ href: "#/", label: "Home" }, { href: `#/${kind}`, label: meta.title }, { href: `#/${kind}/${year}`, label: YEAR_LABEL[year] }, { label: SEM_LABEL[semester] }])}
    <h1 class="section-title">${meta.title}</h1>
      ...
      <div class="preview-sheet">
        <div class="preview-bar">
          ...
          <div class="inline-actions">
            <a id="preview-download" class="btn ghost" target="_blank">Download</a>
...
function adminPanelView() {
  const cOpts = catalog.subjects.map(s => (s.chapters||[]).map(c => `<option value="${c.id}">${s.year}Y ${s.semester}S - ${escapeHtml(s.name)} - ${escapeHtml(c.name)}</option>`).join("")).join("");
  const fRows = db.files.slice().reverse().map(f => { 
    const c = db.chapters.find(x=>x.id===f.chapterId); 
    const s = c?db.subjects.find(x=>x.id===c.subjectId):null; 
    return `<tr><td>${f.kind}</td><td>${s?escapeHtml(s.name):"-"}</td><td>${c?escapeHtml(c.name):"-"}</td><td><a href="${f.driveUrl}" target="_blank">${escapeHtml(f.title)}</a></td><td><button class="danger" data-del-file="${f.id}">Del</button></td></tr>`; 
  }).join("");
  ...
  return `
    <div class="admin-wrap">
      <section class="panel">
        <h2>Add File Link</h2>
        <form id="file-form">
          <select name="cId">${cOpts}</select><br><br>
          <select name="k"><option value="notes">Notes</option><option value="textbook">Textbook</option><option value="ppt">PPT</option></select><br><br>
          <input name="t" placeholder="Title" required/><br><br>
          <input name="u" placeholder="Drive Link" required/><br><br>
          <button type="submit">Save</button>
        </form>
        <table class="table" style="margin-top:15px"><tbody>${fRows}</tbody></table>
      </section>
      <section class="panel">
        <h2>Settings</h2>
        <form id="set-form">
          <input name="pw" type="password" placeholder="New Password (optional)"/><br><br>
          <button type="submit">Save</button> <button type="button" class="ghost" id="lo-btn">Logout</button>
        </form>
      </section>
      <section class="panel">
        <h2>Add Subject</h2>
        <form id="sub-form">
          <input name="y" placeholder="Year 1-4" required/> 
          <input name="s" placeholder="Sem 1-8" required/> 
      ...
      <section class="panel">
        <h2>Add Chapter</h2>
        <form id="ch-form">
          <select name="sId">${catalog.subjects.map(s=>`<option value="${s.id}">${escapeHtml(s.name)}</option>`).join("")}</select> 
          <input name="n" placeholder="Name" required/> 
          <button type="submit">Add</button>
        </form>
      </section>
...
function renderAdmin() {
  ...
  document.getElementById("lo-btn").onclick = () => { adminOk = false; sessionStorage.removeItem("dt-admin"); render(); };
  document.getElementById("set-form").onsubmit = (e) => {
    e.preventDefault(); 
    const pw = new FormData(e.target).get("pw"); 
    if (pw.length > 3) db.settings.adminPassword = pw; 
    saveDb(db); render(); toast("Saved");
  };
  document.getElementById("sub-form").onsubmit = (e) => {
    e.preventDefault(); 
    const fd = new FormData(e.target); 
    db.subjects.push({id: uid("s"), year: Number(fd.get("y")), semester: Number(fd.get("s")), name: fd.get("n").trim(), order: 99}); 
    saveDb(db); render(); toast("Added");
  };
  document.getElementById("ch-form").onsubmit = (e) => {
    e.preventDefault(); 
    const fd = new FormData(e.target); 
    db.chapters = db.chapters || []; 
    db.chapters.push({id: uid("c"), subjectId: fd.get("sId"), name: fd.get("n").trim(), order: 99}); 
    saveDb(db); render(); toast("Added");
  };
  document.getElementById("file-form").onsubmit = (e) => {
    e.preventDefault(); 
    const fd = new FormData(e.target); 
    const urls = driveUrls(fd.get("u")); 
    if (!urls) return toast("Invalid Link"); 
    db.files = db.files || []; 
    db.files.push({id: uid("f"), chapterId: fd.get("cId"), kind: fd.get("k"), title: fd.get("t").trim(), driveUrl: urls.driveUrl, previewUrl: urls.previewUrl, downloadUrl: urls.downloadUrl}); 
    saveDb(db); render(); toast("Saved");
  };
  app.querySelectorAll("[data-del-file]").forEach(b => b.onclick = () => {
    if (!confirm("Delete?")) return; 
    db.files = db.files.filter(f => f.id !== b.dataset.delFile); 
    saveDb(db); render();
  });
}
...
function bindPreview() {
  ...
  if (!m) return; 
  document.getElementById("preview-close").onclick = () => { m.hidden = true; document.getElementById("preview-frame").src = ""; };
  app.querySelectorAll("[data-open-file]").forEach(b => {
    b.onclick = () => {
      const f = JSON.parse(decodeURIComponent(b.dataset.openFile)); 
      document.getElementById("preview-title").textContent = f.title; 
      document.getElementById("preview-download").href = f.downloadUrl; 
      document.getElementById("preview-frame").src = f.previewUrl; 
      m.hidden = false;
    };
  });
}
...
function render() {
  ...
  if (!p[0]) app.innerHTML = homeView();
  else if (p[0] === "admin") renderAdmin();
  else if (!p[1]) app.innerHTML = yearView(p[0]);
  else if (!p[2]) app.innerHTML = semesterView(p[0], Number(p[1]));
  else { app.innerHTML = filesView(p[0], Number(p[1]), Number(p[2])); bindPreview(); }
}

async function syncCloud() {
  if (!supabase) return; 
  try {
    const { data } = await supabase.from('app_data').select('db_json').eq('id', 1).single(); 
