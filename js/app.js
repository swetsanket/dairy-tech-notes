...
function renderAdmin() {
  ...
  app.querySelectorAll("[data-rename-chapter]").forEach((btn) => { btn.onclick = () => { const name = prompt("New chapter name"); if (!name) return; const c = db.chapters.find((x) => x.  document.body.appendChild(el);
  setTimeout(() => el.remove(), 2200);
}
...
function renderAdmin() {
  ...
  app.querySelectorAll("[data-rename-subject]").forEach((btn) => { btn.onclick = () => { const name = prompt("New subject name"); if (!name) return; const s = db.subjects.find((x) => x.id === btn.dataset.renameSubject); if (s) s.name = name.trim(); saveDb(db); refresh(); render(); }; });
  app.querySelectorAll("[data-del-subject]").forEach((btn) => { btn.onclick = () => { if (!confirm("Delete this subject and its chapters/files?")) return; const id = btn.dataset.delSubject; const chapterIds = db.chapters.filter((c) => c.subjectId === id).map((c) => c.id); db.files = (db.files || []).filter((f) => !chapterIds.includes(f.chapterId)); db.chapters = db.chapters.filter((c) => c.subjectId !== id); db.subjects = db.subjects.filter((s) => s.id !== id); saveDb(db); refresh(); render(); }; });
  app.querySelectorAll("[data-rename-chapter]").forEach((btn) => { btn.onclick = () => { const name = prompt("New chapter name"); if (!name) return; const c = db.chapters.find((x) => x.id === btn.dataset.renameChapter); if (c) c.name = name.trim(); saveDb(  el.textContent = msg;
  document.body.appendChild(el);
  setTimeout(() => el.remove(), 2200);
}
...
function renderAdmin() {
  ...
  app.querySelectorAll("[data-rename-subject]").forEach((btn) => { btn.onclick = () => { const name = prompt("New subject name"); if (!name) return; const s = db.subjects.find((x) => x.id === btn.dataset.renameSubject); if (s) s.name = name.trim(); saveDb(db); refresh(); render(); }; });
  app.querySelectorAll("[data-del-subject]").forEach((btn) => { btn.onclick = () => { if (!confirm("Delete this subject and its chapters/files?")) return; const id = btn.dataset.delSubject; const chapterIds = db.chapters.filter((c) => c.subjectId === id).map((c) => c.id); db.files = (db.files || []).filter((f) => !chapterIds.includes(f.chapterId)); db.chapters = db.chapters.filter((c) => c.subjectId !== id); db.subjects = db.subjects.filter((s) => s.id !== id); saveDb(db); refresh(); render(); }; });
  app.querySelectorAll("[data-rename-chapter]").forEach((btn) => { btn.onclick = () => { const name = prompt("New chapter name"); if (!name) return; const c = db.chapters.find((x) => x.id === btn.dataset.renameChapter); if (c) c.name = name.trim(); saveDb(db); refresh(); render(); }; });
  app.querySelectorAll("[data-del-chapter]").forEach(    driveUrl: "https://drive.google.com/file/d/" + id + "/view",
    previewUrl: "https://drive.google.com/file/d/" + id + "/preview",
    downloadUrl: "https://drive.google.com/uc?export=download&id=" + id
  };
}
